...
async function getDetailBBSData(id: number) {
  const respose = await fetch(`http://localhost:3000/api/post/${id}`, {
    cache: "no-store",
  });

  const bbsDetailData: BBSData = await respose.json();

  return bbsDetailData;
}

const BBSDetailPage = async ({ params }: { params: { bbsId: number } }) => {
  const id = await params.bbsId;
  const bbsDetailData = await getDetailBBSData(id);
